...
0x00   |    version    |    request    |       package length (1)     |                        package length (2)                     |
...
class FtpProtocol:
    ...
    def __init__(self, request, main_content_len, version=1):
        if (version != 1):
        ...
        if (not isinstance(main_content), int or main_content_len < 0):
            raise ProtocalError("Content type error")

        self.content_len = main_content
