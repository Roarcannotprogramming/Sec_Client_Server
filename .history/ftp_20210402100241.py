...
class FtpProtocol:
    ...
    def __send(self, pack):
        package_len = ord(pack(1)) + (ord(pack(2)) << 8) + (ord(pack(3)) << 16)
        request = ord(pack(0)) >> 4
        fragment = ord(pack(7)) >> 7
        last_fragment = (ord(pack(7)) >> 6) & 1
        fragment_id = ord(pack(4)) + (ord(pack(5)) << 8) + (ord(pack(6)) << 16) + ((ord(pack(7)) & 0x3f) << 24)
...
f = FtpProtocol(0).feed()
f.send(NULL)
f.send((b"a123123414523523",1))
